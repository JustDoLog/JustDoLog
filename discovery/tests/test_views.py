...
class DiscoveryViewTests(TestCase):
    ...
    def test_popular_bloggers_view(self):
        ...
        other_user_blog = next(
            (blogger for blogger in bloggers if blogger.owner == self.other_user), None
        )
        self.assertIsNotNone(other_user_blog)
        self.assertTrue(
            other_user_blog.is_followed
        )  # following_dict 대신 is_followed 확인
    ...
    def test_tagged_posts_view(self):
        ...
        response = self.client.get(
            reverse("tagged_posts", kwargs={"tag_name": "python"})
        )
        self.assertEqual(response.status_code, 200)
        ...
        # django 태그로 필터링
        response = self.client.get(
            reverse("tagged_posts", kwargs={"tag_name": "django"})
        )
        self.assertIn(self.post, response.context["posts"])
        self.assertNotIn(self.other_post, response.context["posts"])
