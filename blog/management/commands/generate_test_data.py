...
class Command(BaseCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 태그 풀 초기화
        self.tags_pool = self.generate_tags()

        self.is_test = kwargs.pop(
            "test_mode", False
        )  # kwargs에서 test_mode를 꺼내고 제거
        super().__init__(*args, **kwargs)
        self.fake = Faker("ko_KR")
        ...
        if os.path.exists(self.profile_images_dir):
            self.profile_images = [
                f
                for f in os.listdir(self.profile_images_dir)
    ...
    def calculate_engagement_metrics(self, created_at):
        ...
        if days_old < 1:  # 오늘 (일간)
            return {"views": random.randint(1, 49), "likes": random.randint(1, 49)}
        elif days_old < 7:  # 이번 주 (주간)
            return {"views": random.randint(50, 99), "likes": random.randint(50, 99)}
        ...
        try:
            ...
            try:
                self.s3_client.head_object(
                    Bucket=settings.AWS_STORAGE_BUCKET_NAME, Key=filepath
                )
            ...
            public_url = (
                f"http://localhost:9000/{settings.AWS_STORAGE_BUCKET_NAME}/{filepath}"
            )
        ...
        try:
            ...
            for i in range(0, len(users), 10):  # 10명의 사용자씩 처리
                ...
                with transaction.atomic():
                    ...
                    self.stdout.write(
                        f"사용자 {i+1}-{i+len(user_batch)}의 포스트 생성 완료"
                    )
            ...
            for i in range(0, len(users), 5):  # 5명의 사용자씩 처리
                ...
                with transaction.atomic():
                    ...
                    self.stdout.write(
                        f"사용자 {i+1}-{i+len(user_batch)}의 활동 기록 생성 완료"
                    )
    ...
    def create_users(self, num_users):
        ...
        for i in range(num_users):
            ...
            # 추가 프로필 정보 설정
            user.github_url = (
                f"https://github.com/{username}" if random.random() < 0.7 else ""
            )
            user.twitter_url = (
                f"https://twitter.com/{username}" if random.random() < 0.4 else ""
            )
            user.facebook_url = (
                f"https://facebook.com/{username}" if random.random() < 0.3 else ""
            )
            user.homepage_url = (
                f"https://{username}.github.io" if random.random() < 0.2 else ""
            )
    ...
    def create_posts(self, users):
        ...
        for user in users:
            ...
            for _ in range(num_posts):
                ...
                # 태그 적용
                num_tags = random.randint(1, 5)  # 각 포스트당 1-5개의 태그
                selected_tags = random.sample(self.tags_pool, num_tags)
                post.tags.add(*selected_tags)  # django-taggit의 add 메서드 사용
            ...
            if available_posts:
                for post in available_posts:
                    ...
                    if post.likes < post.target_likes:
                        likes_to_create.append(PostLike(user=user, post=post))
                        post.likes += 1
    ...
    def assign_random_profile_image(self, user):
        ...
        with open(image_path, "rb") as img_file:
            user.profile_image.save(
                image_name, File(img_file), save=True  # 원본 파일명 유지
            )
