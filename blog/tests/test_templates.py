...
from freezegun import freeze_time  # 상단에 추가

User = get_user_model()


class BlogTemplateTests(TestCase):
    @freeze_time("2025-02-11")  # 날짜 고정
    def setUp(self):
        """테스트 설정"""
        self.client = Client()
    ...
    def test_post_detail_page_elements(self):
        ...
        # 날짜 형식이 템플릿에서 사용하는 형식과 일치하는지 확인
        expected_date = self.post.created_at.strftime("%Y년 %m월 %d일")
