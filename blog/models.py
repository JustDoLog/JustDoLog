    ...
    def popular(self):
        ...
        return (
            self.annotate(
                ...
                views_count=Sum(
                    "posts__views", filter=Q(posts__status="published"), default=0
                ),
                likes_count=Sum(
                    "posts__likes", filter=Q(posts__status="published"), default=0
                ),
            ...
            .filter(posts_count__gt=0)  # 게시글이 있는 블로그만
            .select_related("owner")  # owner 정보 미리 로드
...
class Blog(models.Model):
    ...
    def get_tags_with_count(self):
        ...
        return (
            Tag.objects.filter(post__blog=self, post__status="published")
            .annotate(
    ...
    def update_stats(self):
        ...
        stats = self.posts.filter(status="published").aggregate(
            total_posts=Count("id"), total_views=Sum("views"), total_likes=Sum("likes")
        )
...
class Post(models.Model):
    ...
    def get_like_url(self):
        ...
        return reverse(
            "toggle_like",
            kwargs={"username": self.blog.owner.username, "slug": self.slug},
        )
...
class PostReadManager(models.Manager):
    ...
    def record_read(self, user, post):
        ...
        read_record, created = self.get_or_create(
            user=user, post=post, defaults={"updated_at": timezone.now()}
        )
        if not created:
